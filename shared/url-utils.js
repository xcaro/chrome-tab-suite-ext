...
// ── FB About URL detection ─────────────────────────────────
const _FB_IGNORED = new Set([
  '/', '/home', '/watch', '/marketplace', '/groups', '/events', '/gaming',
  '/pages', '/ads', '/business', '/help', '/login', '/logout', '/settings',
  '/privacy', '/policies', '/notifications', '/messages', '/inbox', '/friends',
  '/saved', '/memories', '/feeds', '/explore', '/reels', '/stories', '/search',
  '/fundraisers', '/weather', '/jobs', '/video', '/live',
]);

const _FB_PROFILE_TABS = new Set([
  'posts', 'photos', 'videos', 'friends', 'followers',
  'following', 'reels', 'groups', 'reviews',
]);

export function getFbAboutUrl(url) {
  try {
    const p    = new URL(url);
    if (!p.hostname.includes('facebook.com')) return null;
    const path = p.pathname;

    if (path === '/profile.php') {
      if (p.searchParams.get('sk') === 'about') return null;
      const n = new URL(url);
      n.searchParams.set('sk', 'about');
      return n.toString();
    }
    ...
    const first = '/' + segs[0];
    if (_FB_IGNORED.has(first) || _FB_IGNORED.has(first.toLowerCase())) return null;
    if (segs.length === 1) return `https://www.facebook.com/${segs[0]}/about`;
    if (segs[1] === 'about') return null;
    if (segs.length === 2 && _FB_PROFILE_TABS.has(segs[1])) {
      return `https://www.facebook.com/${segs[0]}/about`;
    }
    return null;
  } catch { return null; }
}
